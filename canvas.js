...
function createTextItem(content = 'Double-click to edit text...', x = null, y = null, width = null, height = null, fromDatabase = false) {
    ...
    item.addEventListener('blur', () => {
        console.log('Text item blur - saving content');
        console.log('Current text content:', item.textContent);
        console.log('Current innerText:', item.innerText);
        console.log('Current innerHTML:', item.innerHTML);
        
        // Clear any pending save timeout
        if (item.saveTimeout) {
            clearTimeout(item.saveTimeout);
            item.saveTimeout = null;
        }
        
        item.classList.remove('editing');
        item.contentEditable = false;
        
        // Save immediately with current content
        console.log('Saving on blur - content:', item.textContent);
        saveItemToDatabase(item);
    });
    ...
    item.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' && !e.shiftKey) {
            e.preventDefault();
            console.log('Enter key pressed - blurring text item');
            item.blur();
        }
    });
    ...
    item.addEventListener('input', (e) => {
        ...
        item.saveTimeout = setTimeout(() => {
            console.log('Auto-saving text content:', item.textContent);
            saveItemToDatabase(item);
...
function getItemContent(item) {
    ...
    switch (item.dataset.type) {
        case 'image':
            ...
            break;
        case 'video':
            content = item.querySelector('video').src;
            break;
        case 'text':
            content = item.textContent;
            console.log('Text content extracted:', content);
            break;
        case 'code':
            content = item.querySelector('iframe').srcdoc;
            break;
        case 'drawing':
            content = item.querySelector('path').getAttribute('d');
            break;
        default:
            content = '';
    }
    
    console.log('Final content for', item.dataset.type, ':', content);
    return content;
}
...
// Test function for debugging text content issues
function testTextEditing() {
    ...
    if (textItems.length === 0) {
        console.log('No text items found. Please add a text item first.');
        return;
    }
    
    const textItem = textItems[0];
    console.log('Testing text item:', textItem);
    console.log('Current content:', textItem.textContent);
    console.log('Dataset:', textItem.dataset);
    
    // Test content extraction
    const content = getItemContent(textItem);
...
window.testText = function() {
    ...
    if (textItems.length > 0) {
        ...
        console.log('First text item:', item);
        ...
        console.log('No text items found');
    }
};
...
window.testBasics = function() {
    ...
    console.log('getItemContent function:', typeof getItemContent);
    console.log('saveItemToDatabase function:', typeof saveItemToDatabase);
};
...
window.simpleTest = function() {
    ...
    console.log('Supabase available:', typeof supabaseClient !== 'undefined');
};

console.log('Canvas.js loaded completely');